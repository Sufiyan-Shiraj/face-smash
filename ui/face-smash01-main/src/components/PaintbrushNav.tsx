...
export const PaintbrushNav: React.FC<PaintbrushNavProps> = ({
  currentView,
  onNavigate,
  soundMuted,
  onToggleSound,
  ...
  return (
    <header className="w-full relative z-40 select-none px-3 sm:px-6 md:px-8 h-[64px] sm:h-[70px] md:h-[74px] flex items-center justify-between overflow-visible">
      ...
      >
        ...
        {/* Left Brand: Angled slightly along the upward left slope */}
        <div
          onClick={() => {
            sound.playClick();
            onNavigate('upload');
        ...
        >
          <CrownDoodle size={26} color="#facc15" className="group-hover:rotate-12 transition-transform duration-200 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] -rotate-6" />
        ...
        <nav className="flex items-center gap-1.5 sm:gap-2.5 md:gap-3">
          {/* Upload Tab: Rides top-left crest of wave */}
          <button
            onClick={() => {
              sound.playClick();
              onNavigate('upload');
          ...
          {/* Scan 3D Tab: Rides mid-slope downward transition */}
          <button
            onClick={() => {
              sound.playClick();
              onNavigate('scan');
          ...
          {/* Smash Lab Tab: Rides bottom trough curving upward */}
          <button
            onClick={() => {
              sound.playClick();
              onNavigate('smash');
        ...
        {/* Right: Sound & Music FX Badge tilted along the steep right upward flick */}
